...
(function ($) {
    $.fn.PeoplePicker = function () {
        ...
        return this.each(function () {
            ...
            var $resultList = $peoplePicker.find(".ms-PeoplePicker-resultList");
            ...
            $results.on('click', '.ms-PeoplePicker-result', function (event) {
                ...
                var personaHTML = '<div class="ms-PeoplePicker-persona">' +
                      '<div class="ms-Persona ms-Persona--xs ms-Persona--square">' +
                                         '<div class="ms-Persona-imageArea">' +
                                           ...
                                           '<img class="ms-Persona-image" src="'+ selectedImg +'"">' +
                                         '</div>' +
                ...
                var personaListItem = '<li class="ms-PeoplePicker-selectedPerson">' +
                        '<div class="ms-Persona ms-Persona--square">' +
                               '<div class="ms-Persona-imageArea">' +
                                 '<i class="ms-Persona-placeholder ms-Icon ms-Icon--person"></i>' +
                                  '<img class="ms-Persona-image" src="' + selectedImg + '"><div class="ms-Persona-presence"></div>' +
                               '</div>' +
                               '<div class="ms-Persona-details">' +
                                  '<div class="ms-Persona-primaryText">' + selectedName + '</div>' +
            ...
            /**
             * Get all the users for this search query
             */
            $searchField.on('input', function(event) {
                event.stopPropagation();
                if ($(this).val().length > 2) {
                    $('.searchCallout').stop().clearQueue().hide();
                    //$('.searchCallout').hide();
                    ...
                    clearResultList();
                    $('.searchCallout').stop().clearQueue().hide();
                    $('.searchCallout').delay(1500).show(0);
                }
    ...
    /**
     * Add the found user to the resultList
     * @param {} user : the SharePoint user found
     * @returns {} 
     */
    function addUserToResult(user) {
        var html =
        ...
        '<div class="ms-Persona-imageArea">' +
        '<i class="ms-Persona-placeholder ms-Icon ms-Icon--person"></i>' +
        '<img class="ms-Persona-image" src="http://placehold.it/50?text=Photo" />' +
        '<div class="ms-Persona-presence"></div>' +
        '</div>' +
        '<div class="ms-Persona-details">' +
        ...
        '</button>' +
        '</li>';
        $('.ms-PeoplePicker-resultList').append(html);
    }

    /**
     * Clear the persons already found
     * @returns {} 
     */
    function clearResultList() {
        $('.ms-PeoplePicker-resultList').empty();
    }

    /**
    * Asks Sharepoint for the users with the searchString in their name
