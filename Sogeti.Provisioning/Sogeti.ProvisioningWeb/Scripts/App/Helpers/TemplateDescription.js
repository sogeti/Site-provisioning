...
$(function() {
    $("#SelectedSiteCreationTemplate").on('change', function() {

        $('#SelectedTemplateDescription').empty();
        if ($("#SelectedSiteCreationTemplate").val() != 0) {

            var SelectedTemplate = $(this).val();

            var requestData = {
                value: $.trim($('#SelectedSiteCreationTemplate').val())
            };

            if (SelectedTemplate.length > 0) {

                $.getJSON('/SiteCollection/GetTemplateDescription', {
                    SelectedSiteCreationTemplate: $("#SelectedSiteCreationTemplate").val()
                })
                 .done(function (data) {
                     $("#SelectedTemplateDescription").html(data);
                     $('#SelectedTemplateDescriptionBox').addClass("template-description-fadein");
                 })
                 .fail(function (jqxhr, textStatus, error) {
                     var err = textStatus + ", " + error;
                     console.log("Request Failed: " + err);
                     $("#SelectedTemplateDescription").html("An error occurred retrieving the description, please try again later.");
                 });


            } else {
                $('#SelectedTemplateDescription').empty();
                $('#SelectedTemplateDescriptionBox').removeClass("template-description-fadein");
            }
        }
    });
});
